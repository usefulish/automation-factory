...
```
ChatGPT GPT Action  ──HTTPS/REST+Bearer──▶  swamp-chatgpt-gateway  ──WebSocket (bearer.<token> subprotocol)──▶  swamp serve
```
...
`swamp serve` via the `bearer.<token>` WebSocket subprotocol. Swamp's grant
model then scopes what the caller may `read`/`run`, so:
...
Issue the token and its grant out-of-band:

```bash
# 1. a server token for the ChatGPT user
swamp access token create chatgpt --json
#    -> { "name": "...", "secret": "...", "token": "<name>.<secret>" }

# 2. scope it: allow read + run on the approved workflow(s) only
swamp access grant allow chatgpt-user run  workflow:@usefulish/mintlify-docs
swamp access grant allow chatgpt-user read workflow:@usefulish/mintlify-docs
# deny everything else explicitly if you want fail-closed
swamp access grant deny  chatgpt-user run  workflow:*
```
...
GPT Actions reach the gateway over the public internet, so lock the gateway down
to ChatGPT's egress (or put it behind your own tunnel / access proxy). The
bearer token is the only secret a caller needs — treat the gateway as an
authenticating proxy, not a public service.
